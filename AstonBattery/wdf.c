...
EVT_WDF_DRIVER_DEVICE_ADD AstonBatteryDriverDeviceAdd;
EVT_WDF_DEVICE_SELF_MANAGED_IO_INIT  AstonBatterySelfManagedIoInit;
EVT_WDF_DEVICE_SELF_MANAGED_IO_CLEANUP  AstonBatterySelfManagedIoCleanup;
EVT_WDF_DEVICE_QUERY_STOP AstonBatteryQueryStop;
EVT_WDF_DEVICE_PREPARE_HARDWARE AstonBatteryDevicePrepareHardware;
EVT_WDFDEVICE_WDM_IRP_PREPROCESS AstonBatteryWdmIrpPreprocessDeviceControl;
EVT_WDFDEVICE_WDM_IRP_PREPROCESS AstonBatteryWdmIrpPreprocessSystemControl;
WMI_QUERY_REGINFO_CALLBACK AstonBatteryQueryWmiRegInfo;
WMI_QUERY_DATABLOCK_CALLBACK AstonBatteryQueryWmiDataBlock;
EVT_WDF_DRIVER_UNLOAD AstonBatteryEvtDriverUnload;
EVT_WDF_OBJECT_CONTEXT_CLEANUP AstonBatteryEvtDriverContextCleanup;

//---------------------------------------------------------------------- Pragmas

#pragma alloc_text(INIT, DriverEntry)
#pragma alloc_text(PAGE, AstonBatterySelfManagedIoInit)
#pragma alloc_text(PAGE, AstonBatterySelfManagedIoCleanup)
#pragma alloc_text(PAGE, AstonBatteryQueryStop)
#pragma alloc_text(PAGE, AstonBatteryDriverDeviceAdd)
#pragma alloc_text(PAGE, AstonBatteryDevicePrepareHardware)
#pragma alloc_text(PAGE, AstonBatteryWdmIrpPreprocessDeviceControl)
#pragma alloc_text(PAGE, AstonBatteryWdmIrpPreprocessSystemControl)
#pragma alloc_text(PAGE, AstonBatteryQueryWmiRegInfo)
#pragma alloc_text(PAGE, AstonBatteryQueryWmiDataBlock)
#pragma alloc_text(PAGE, AstonBatteryEvtDriverUnload)
#pragma alloc_text(PAGE, AstonBatteryEvtDriverContextCleanup)
...

{
	...
	WDF_DRIVER_CONFIG_INIT(&DriverConfig, AstonBatteryDriverDeviceAdd);
	DriverConfig.EvtDriverUnload = AstonBatteryEvtDriverUnload;
	...
	DriverAttributes.EvtCleanupCallback = AstonBatteryEvtDriverContextCleanup;
...

{
	...
	PnpPowerCallbacks.EvtDevicePrepareHardware = AstonBatteryDevicePrepareHardware;
	PnpPowerCallbacks.EvtDeviceSelfManagedIoInit = AstonBatterySelfManagedIoInit;
	PnpPowerCallbacks.EvtDeviceSelfManagedIoCleanup = AstonBatterySelfManagedIoCleanup;
	PnpPowerCallbacks.EvtDeviceQueryStop = AstonBatteryQueryStop;
	...
	Status = WdfDeviceInitAssignWdmIrpPreprocessCallback(
		DeviceInit,
		AstonBatteryWdmIrpPreprocessDeviceControl,
	...
	Status = WdfDeviceInitAssignWdmIrpPreprocessCallback(
		DeviceInit,
		AstonBatteryWdmIrpPreprocessSystemControl,
...

{
	...
	BattInit.QueryTag = AstonBatteryQueryTag;
	BattInit.QueryInformation = AstonBatteryQueryInformation;
	BattInit.SetInformation = AstonBatterySetInformation;
	BattInit.QueryStatus = AstonBatteryQueryStatus;
	BattInit.SetStatusNotify = AstonBatterySetStatusNotify;
	BattInit.DisableStatusNotify = AstonBatteryDisableStatusNotify;
	...
	DevExt->WmiLibContext.QueryWmiRegInfo = AstonBatteryQueryWmiRegInfo;
	DevExt->WmiLibContext.QueryWmiDataBlock = AstonBatteryQueryWmiDataBlock;
...
_Use_decl_annotations_
VOID
AstonBatterySelfManagedIoCleanup(
	WDFDEVICE Device
)
...
Routine Description:
	...
	AstonBattery is designed to fail a rebalance operation, for reasons described
	below. Note however that this approach must *not* be adopted by an actual
	battery driver.

	AstonBattery unregisters itself as a Battery driver by calling
	...
	be delivered to AstonBattery. The IO Queue employed by AstonBattery is power managed,
	it causes these IOCTLs to be pended when AstonBattery is not in D0. Now if the
	device attempts to do a shutdown while an IOCTL is pended in AstonBattery, it
	would result in a 0x9F bugcheck. By opting out of PNP rebalance operation
	AstonBattery circumvents this issue.
...
_Use_decl_annotations_
NTSTATUS
AstonBatteryDevicePrepareHardware(
	WDFDEVICE Device,
...

{
	...
	AstonBatteryPrepareHardware(Device);
...

{
	...
	if (InstanceLengthArray == NULL) {
		...
		goto AstonBatteryQueryWmiDataBlockEnd;
	}
...
AstonBatteryQueryWmiDataBlockEnd:
	Trace(TRACE_LEVEL_INFORMATION, SURFACE_BATTERY_TRACE, "Leaving %!FUNC!: Status = 0x%08lX\n", Status);
	return Status;
}
...
VOID
AstonBatteryEvtDriverContextCleanup(
